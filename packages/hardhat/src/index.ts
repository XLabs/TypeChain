...
import type { PublicConfig as RunTypeChainConfig } from '@xlabs-xyz/typechain'
...
subtask(TASK_TYPECHAIN_GENERATE_TYPES)
  ...
  .setAction(async ({ compileSolOutput, quiet }, { config, artifacts }) => {
    ...
    const { glob } = await import('@xlabs-xyz/typechain')
    ...
    const { runTypeChain } = await import('@xlabs-xyz/typechain')
