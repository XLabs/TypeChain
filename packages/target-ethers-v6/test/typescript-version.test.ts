...
describe('Ethers target constructor', () => {
  ...
  it('panics when TypeScript version is lower than 4.3', () => {
    ...
    }).toThrow('@xlabs-xyz/typechain-ethers-v6 1.2.3 needs TypeScript version 4.3 or newer.')

    const message = consoleErrorMock.calls[0].args[0]

    expect(message).toEqual(expect.stringMatching('@xlabs-xyz/typechain-ethers-v6 1.2.3 needs TypeScript version 4.3 or newer.'))
